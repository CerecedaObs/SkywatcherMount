'''
 This code uses indi_eqmod_telescope to emulate a synscan control for skywatcher mounts.
 Most of it was taken from indi forums.
'''
...
TELESCOPE_SIMULATION=False
TELESCOPE_SIMPROP="SIMULATION"

target = '/tmp/mountcommands.txt'
...
def Move(axis='RA', to='left', vel=8):
    '''
      axis: RA, DEC
      to  : left, right
      vel : = from 0 to 9, incl
    '''
    axisdic = {'ra':16, 'dec':17}
    dirdic = {'l':36, 'r':17}
    axis = axis.lower()
    to = to[0].lower()
    vel = int(vel)
    if not axis in axisdic.keys(): 
        print('ERROR in axis "%s"'%axis)
        return
    if not to   in dirdic .keys():
        print('ERROR in direction "%s%'%to)
        return
    if vel>9: vel = 9
    if vel <0: vel = 0

    command = [ord('P'), 2, axisdic[axis], dirdic[to], vel]
    try:
      chars = command
      ...
      if USE_SERIAL: os.remove(DEVICE_PORT)

def Stop():
    Move(axis='RA', to='left', vel=0)
    Move(axis='DEC', to='left', vel=0)
        ...
        if os.path.isfile(target):
            with open(target, 'r') as f:
                ...
                if len(l) == 4:
                    ...
                    l = l[:-1]
                if not len(l) == 3:
                    print('ERROR: len(l) = ', len(l))
                    time.sleep(1)
                    continue
                axis, to, vel = l
                Move(axis, to, vel)
                if forlong is not None:
                  print('Waiting for %1.2f seconds...'%forlong)
                  time.sleep(forlong)
                  Stop()
            os.system('rm %s'%target)
        time.sleep(0.1)
